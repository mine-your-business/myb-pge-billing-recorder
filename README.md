...
```sh
aws ssm put-parameter \
    --name "/portlandgeneral/billing/username" \
    --type "String" \
    --value "0x1234abcd"
```
...
This project leverages Google Sheets for maintaining a history of the electricity billing data retrieved from Portland General.

The current column settings assume you have a sheet that looks like this (the names don't need to match, only the number of columns) for the billing data (`Timestamp,Billing Cycle Day,Number of Billing Days,Last Read Date,Bill to Date Amount,Projected Amount,Min Projected Amount,Max Projected Amount,Billing Progress,Last Amt,Current Amt,Current kWh Total,Issue Date,Due Date,Period Start,Period End`):

![Billing Sheet](images/billing_spreadsheet.png)
...
`NOTE`: This command syntax works for all AWS Lambda functions; not just the ones you deploy using SAM.

```sh
sam logs -n ExampleFunction --stack-name myb-pge-billing-recorder --tail
```
...
```sh
aws cloudformation delete-stack --stack-name myb-pge-billing-recorder
```
